...
TfLiteStatus SetupAccelerometer(tflite::ErrorReporter* error_reporter) {
  ...
  if (imu_initial() != HX_DRV_IMU_PASS) {
    TF_LITE_REPORT_ERROR(error_reporter, "imu setup fail");
    return kTfLiteError;
  }

  TF_LITE_REPORT_ERROR(error_reporter, "imu setup done");
...
bool ReadAccelerometer(tflite::ErrorReporter* error_reporter, float* input,
  ...
  available_count = imu_accelerationAvailableCount();
